...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        # 在调用 sceneLoadTypeCallbacks 前强制启用 eval
        args.eval = True  # 强制加载测试集
        ...
        print(f"[DEBUG] scene_info.test_cameras: {len(scene_info.test_cameras)}")
        print(f"[DEBUG] scene_info.train_cameras: {len(scene_info.train_cameras)}")
        ...
        if not self.loaded_iter:
            ...
            print(f"[DEBUG] camlist before json dump: {len(camlist)}")
        ...
        for resolution_scale in resolution_scales:
            ...
            print(f"[DEBUG] self.test_cameras[{resolution_scale}]: {len(self.test_cameras[resolution_scale])}")

            # 伪相机生成逻辑
            pseudo_cams = []
            if args.source_path.find('llff') != -1:
                from utils.pose_utils import generate_random_poses_llff
            ...
            # if len(self.train_cameras[resolution_scale]) > 0 and pseudo_poses
            if len(self.train_cameras[resolution_scale]) > 0 and pseudo_poses is not None and len(pseudo_poses) > 0:
                view = self.train_cameras[resolution_scale][0]
                for pose in pseudo_poses:
                    pseudo_cams.append(PseudoCamera(
                        R=pose[:3, :3].T, T=pose[:3, 3],
